...
export function AuthScreen({ onLogin, theme = 'dark', onCancel }: AuthScreenProps) {
  ...
  useEffect(() => {
    ...
    setStatus(
      isRecovery
        ? 'Enter your username, recovery password, and new password.'
        : 'Use your username as your Dicord identity.'
    );
  ...
  useEffect(() => {
    ...
    const count = failedAttempts[cleanUsername] || 0;
    if (count >= 5) {
      fetch('/api/check-recovery', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        ...
        .then((res) => res.json())
        .then((data) => {
          setCanRecover(!!data.hasRecoveryPassword);
        })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onLogin(
        username,
        password,
        isRegister ? displayName : undefined,
        isRegister,
        recoveryPassword || undefined,
        isRecovery
      );
      if (isRecovery) {
        // Clear failed attempts upon successful recovery
        const cleanUsername = username.trim().toLowerCase();
        setFailedAttempts((prev) => ({ ...prev, [cleanUsername]: 0 }));
      }
    } catch (error) {
      if (!isRegister && !isRecovery) {
        ...
        setFailedAttempts((prev) => ({
          ...prev,
          [cleanUsername]: (prev[cleanUsername] || 0) + 1
        }));
      }
        ...
        >
          ...
          {isRecovery ? (
            <div className="flex items-center gap-2 mb-6">
              ...
              <span className="text-sm font-semibold text-theme-text">Account Recovery</span>
            </div>
          ) : (
            <div className="bg-black/5 dark:bg-black/20 p-1 rounded-xl flex mb-6 border border-theme-border">
              <button
                ...
                className={`flex-1 py-2.5 text-sm font-medium rounded-lg transition-all cursor-pointer ${
                  !isRegister && !isRecovery ? 'bg-indigo-500/10 border border-indigo-500/20 text-indigo-600 dark:text-indigo-300 font-bold shadow-md' : 'text-theme-muted hover:text-theme-text'
                }`}
              >
                Log In
              </button>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                placeholder={isRecovery ? 'Enter new password' : 'Password'}
                className="w-full bg-theme-bg/50 border border-theme-border rounded-xl px-4 py-3.5 text-theme-text focus:outline-none focus:border-indigo-500 transition-colors placeholder:text-slate-500"
                required
              />
            </div>
            ...
            {isRegister && (
              <div>
                ...
                <input
                  ...
                  value={recoveryPassword}
                  onChange={(e) => setRecoveryPassword(e.target.value)}
                  placeholder="For resetting password if locked out"
                  className="w-full bg-theme-bg/50 border border-theme-border rounded-xl px-4 py-3.5 text-theme-text focus:outline-none focus:border-indigo-500 transition-colors placeholder:text-slate-500"
                />
                <span className="text-[10px] text-theme-muted ml-1 mt-1 block leading-normal">
                  Write this down safely. It cannot be recovered if lost.
                </span>
              </div>
            )}

            {!isRegister && !isRecovery && canRecover && (
              <div className="flex justify-end select-none animate-fade-in">
                <button
                  type="button"
                  onClick={() => setIsRecovery(true)}
                  className="text-xs text-indigo-500 hover:text-indigo-400 font-semibold transition-colors cursor-pointer flex items-center gap-1.5"
                >
            ...
            >
              {isSubmitting ? 'Please wait' : isRecovery ? 'Recover & Sign In' : isRegister ? 'Create account' : 'Enter chat'}
            </motion.button>
                      ...
                      {devAccounts.map((account, idx) => (
                        ...
                        >
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-2">
                            <div className="bg-amber-500/10 border border-amber-500/20 p-3 rounded-lg flex flex-col justify-center">
                            ...
                            <div className="bg-indigo-500/10 border border-indigo-500/20 p-3 rounded-lg flex flex-col justify-center">
                              <span className="block text-[10px] font-bold text-indigo-400 uppercase tracking-wider mb-0.5">Plaintext Recovery Password</span>
                              <code className="text-sm font-mono font-bold text-indigo-300 select-all">{account.plainRecoveryPassword || '(None)'}</code>
                            </div>
                          </div>
